...
obj <- readRDS(snakemake@input[['gex']])
remo_obj <- readRDS(snakemake@input[['remo']])

# add multiome annotations
remo_obj <- AddMetaData(remo_obj, obj$predicted.subclass, col.name = "multiome_annotation")
remo_obj$prediction_score <- obj$predicted.subclass.score

remo_obj <- FindNeighbors(remo_obj, reduction = "pca", dims = 1:30)
remo_obj <- FindClusters(remo_obj, algorithm = 3, resolution = 2, graph.name = "RNA_snn")

# map multiome annotations to remo clusters
unique_clusters <- as.character(unique(remo_obj$seurat_clusters))
md <- remo_obj[[]]
ct_list <- c()
for (i in seq_along(unique_clusters)) {
    cells_i <- md[md$seurat_clusters == unique_clusters[[i]], ]
    conf <- cells_i[cells_i$prediction_score > 0.5, ]
    top_celltype <- names(sort(table(conf$multiome_annotation), decreasing = TRUE))[[1]]
    ct_list[i] <- top_celltype
}

# add majority multiome celltype to remo seurat clusters
remo_obj$cluster_celltype <- plyr::mapvalues(x = remo_obj$seurat_clusters, from = unique_clusters, to = ct_list)
...
# organ specific
cl <- REMO.v1.GRCh38.tissues$Brain
